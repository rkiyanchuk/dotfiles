...
that if statement (the default). If you only update Conky every 2s, you should 
change it to update_num>3; conversely if you update Conky every 0.5s, you 
should use update_num>10. ALSO, if you change your Conky, is it best to use 
"killall conky; conky" to update it, otherwise the update_num will not be 
reset and you will get an error.
...
settings_table = {
    {
        ...
        name='time',
        -- "arg" is the argument to the stat type, e.g. if in Conky you would 
        -- write ${cpu cpu0}, 'cpu0' would be the argument. If you would not 
        -- use an argument in the Conky variable, use ''.
        arg='%I.%M',
        -- "max" is the maximum value of the ring. If the Conky variable 
        -- outputs a percentage, use 100.
        max=12,
        ...
        start_angle=0,
        -- "end_angle" is the ending angle of the ring, in degrees, clockwise 
        -- from top. Value can be either positive or negative, but must be 
        -- larger than start_angle.
        end_angle=360
    },
    {
        name='time',
...
-- "clock_x" and "clock_y" are the coordinates of the centre of the clock, in 
-- pixels, from the top left of the Conky window.
