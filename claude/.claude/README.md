...
[graphify](https://github.com/safishamsi/graphify) turns a folder of code and
docs into a queryable knowledge graph, so the assistant can answer architectural
questions without re-reading every file. It produces an interactive
`graph.html`, a `GRAPH_REPORT.md`, and a `graph.json` (also usable as an MCP
server).
...
Everything else — flags, querying, PR workflows, MCP server, headless backends,
`.graphifyignore` — see the
[official README](https://github.com/safishamsi/graphify).
